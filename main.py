...
def watch(bot, chat_id, inf):
    try:
        ...
        sn = name
        cursor.execute("""SELECT episode_link_direct,name,episode_name
                        FROM show WHERE episode_name LIKE ? AND name LIKE ?""",
                       (se, sn))
        r = cursor.fetchone()
        bot.sendChatAction(chat_id=chat_id, action=telegram.ChatAction.TYPING)
        bot.sendMessage(chat_id, text="%s\nNow watching: *%s*" %
                        (r[0].strip('[]').replace("'", ""),
                         r[2]), parse_mode=telegram.ParseMode.MARKDOWN)
    except:
        bot.sendPhoto(chat_id,
                      photo="""http://24.media.tumblr.com/
                      c2088c3c87a89f3fe156fcfd9104205e/
                      tumblr_mzl0z5VYmJ1sp41dro4_500.gif""")
        bot.sendMessage(chat_id, text="I can't find it!")


def info(bot, chat_id, series):
    payload = {'q': series}
    r = requests.get("http://api.tvmaze.com/singlesearch/shows",
                     params=payload)
    try:
        jo = r.json()
        bot.sendChatAction(chat_id=chat_id, action=telegram.ChatAction.TYPING)
        ...
        bot.sendMessage(chat_id, text="""Rating: %s\nLanguage: %s\n
                        Runtime: %s min\nStatus: %s\n
                        Genre: %s\nSummary: %s""" %
                        (unicode(jo['rating']), jo['language'],
                         jo['runtime'], jo['status'], genres,
                         TAG_RE.sub('', jo['summary'])))
    except:
        pass
...
def flist(bot, chat_id, inf):
    try:
        ...
        tstr = ""
        bot.sendChatAction(chat_id=chat_id, action=telegram.ChatAction.TYPING)
        for i, row in enumerate(cursor.execute("""SELECT episode_name FROM show
                                WHERE name LIKE %s AND episode_name
                                LIKE \'%%Season %s %%\'""" %
                                (series_name, season))):
